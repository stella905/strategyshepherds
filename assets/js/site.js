(function(){
  ...
  function uid(){
    return 'SS-' + Date.now().toString(36).toUpperCase() + '-' + Math.random().toString(36).slice(2,7).toUpperCase();
  }

  function openLeadModal(intent, source){
    if(!modal || !form) return;
    interestInput.value = intent || 'General enquiry';
    sourceInput.value = source || 'Website CTA';
    pageInput.value = document.title + ' | ' + window.location.pathname;
    const title = document.getElementById('lead-modal-title');
    const intro = document.getElementById('lead-modal-intro');
    if(title) title.textContent = intent || 'Tell us what you need';
    if(intro) intro.textContent = 'Share a few details and we’ll route your enquiry to the right Strategy Shepherds offer.';
  ...
  document.querySelectorAll('[data-lead-intent]').forEach(el=>{
    el.addEventListener('click',e=>{
      e.preventDefault();
      openLeadModal(el.dataset.leadIntent, el.dataset.leadSource || el.textContent.trim());
    });
  });
  ...
  async function submitPayload(payload){
    payload.leadId = payload.leadId || uid();
    payload.utmSource = payload.utmSource || tracking.utmSource;
    payload.utmMedium = payload.utmMedium || tracking.utmMedium;
    payload.utmCampaign = payload.utmCampaign || tracking.utmCampaign;
    payload.pageUrl = window.location.href;
    payload.userAgent = navigator.userAgent;

    if(!endpointReady){
      const saved = JSON.parse(localStorage.getItem('ss_pending_leads') || '[]');
      saved.push(payload);
      localStorage.setItem('ss_pending_leads', JSON.stringify(saved));
      return {ok:true, demo:true, leadId:payload.leadId};
    }

    await fetch(cfg.FORM_ENDPOINT, {
      method:'POST',
      mode:'no-cors',
  ...
  form?.addEventListener('submit',async e=>{
    ...
    try{
      ...
      status.textContent = res.demo
        ? 'Saved locally for testing. Connect the Apps Script endpoint before launch.'
